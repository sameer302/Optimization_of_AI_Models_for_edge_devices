...
- Now lets see how to set this baseline. 
    ...
    Even after doing that our inference process will still have many threads (around 15), by setting restriction on code we have just limited the inference part but the complete pipeline still needs many threads for camera subsystem, GUI/display thread, python runtime / tracing, python background threads, etc. So if we want more granular control on processing then we can try to remove sources of extra threads for e.g. disable GUI. But still we can't bring the entire pipeline to work in just two threads as the runtime itself needs more threads. Now lets set our CPU to assign cores to our code. 
