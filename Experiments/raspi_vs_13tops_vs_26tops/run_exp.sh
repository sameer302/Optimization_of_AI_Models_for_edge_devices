...
timeout $DURATION python ../system_metrics_logger.py \
  --cpu --temp --npu --memory --out "$OUTPUT_DIR/system_metrics.csv" &

timeout $DURATION env HAILO_MONITOR=1 python ./yolo11n_npu.py \
  --csv "$OUTPUT_DIR/yolo11n_npu_results.csv" --input rpi --hef-path /home/sameer/Desktop/optimization_of_ai_models/AIML_models/computer_vision/detection/yolo/yolov11n_hailo8l.hef &
